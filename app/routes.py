...
def index():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(post)
        db.session.commit()
        flash(message='Your post is now live!')
...
def user(username):
    ...
    page = request.args.get('page', 1, type=int)
    posts = users.posts.order_by(Post.timestamp.desc()).paginate(
        page, app.config['POSTS_PER_PAGE'], False)
    next_url = url_for('user', username=users.username, page=posts.next_num) \
        if posts.has_next else None
    prev_url = url_for('user', username=users.username, page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('user.html', user=users, posts=posts.items,
...
def follow(username):
    ...
    db.session.commit()
    flash(message='You are following {}!'.format(username))
    return redirect(url_for('user', username=username))
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash(message='Check your email for the instructions to reset your password')
        return redirect(url_for('login'))
